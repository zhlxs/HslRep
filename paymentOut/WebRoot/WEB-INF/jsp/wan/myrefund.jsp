...
<script type="text/javascript">
        ...
        columns: [{display: '主键', name: 'id', primarykey: true, isHide: true},
            {
                display: '操作', width: 120, link: true, render: function (rowdata) {
                    ...
                    if (rowdata.checkstatus == '0') {
                        html += '<a class="btnPurple newbtn" style="color:#fff;padding:1.5px;4px;"  href="${ctx}/refund/cancleApply?id='
                            + rowdata.id
                            + '" rwpOpts="{ isajax: true }" title="取消退款" button="{}">取消退款</a>&nbsp;|&nbsp;';
                    }
    ...
    rwpTemp.searchDeptUser = function (node) { //按部门查用户
        var queryUrl = rwpTemp.userByDeptlistiniturl, gridManager = $(
            "#myrefundGrid") // 把初始URL获得的数据放到 DIV $("#myrefundGrid")
            .data('rwpUIDatagrid');
    ...
    $(function () {
        ...
        ${ctx}/deptController/listTreeJson?parentid=0',
            ...
            <%--   <dl style="display: none;">
                   ...
                   <dd class="inputWidth">
                       <input style="width: 79px;" name="deptid" type="text" id="deptid" value=""
                              query-options="{ 'fieldName':'d.id', 'whereField':'=', 'fieldType':'String' }"/>
                   </dd>
               </dl>
               <dl style="display: none;">
