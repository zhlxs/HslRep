...
<script type="text/javascript">
	$(function() {
		...
		if ("" != "${photoSample.id}") {
			$("#img_info").attr(
					'src',
					'${ctx}/macPhotoController/getphoto?id=' + $("#id").val()
							+ '&rnd' + Math.random());
		}
	});
			...
			function() {
				if (document.getElementById("describe").value == ""
					...
					document.getElementById("describe").value = document
							.getElementById("photoname").value;
				}
				$("#ajaxform").ajaxSubmit(
						{
							success : function(data) {
								...
								$("#img_info").attr(
										'src',
										'${ctx}/macPhotoController/getphoto?id='
												+ $("#id").val() + '&rnd'
												+ Math.random());
								rwpMenuHelper.rebackLastPage();
							},
...
<div class="fAutoPage">
	<div class="fAutoPage">
		<form action="${ctx}/macPhotoController/saveSample" id="ajaxform"
			...
			<fieldset>
				...
				<div class="formpanel">
					...
					<div class="clear">
						...
						<ul class="DialabelWidth">
							<li class="editor-label"><label for="photoname">数据中文名称</label></li>
							<li class="editor-field"><input data-val="true"
								data-val-length="不能超过100字符" data-val-length-max="100"
								<c:if test="${empty photoSample.photoname }">
									data-val-remote="样图名称已存在" data-val-remote-url="${ctx}/macPhotoController/photoNameExist"
								</c:if>
								data-val-required="不能为空" id="photoname" name="photoname"
								type="text" value="${photoSample.photoname }" /></li>
							<li class="editor-validation"><span
								class="field-validation-valid" data-valmsg-for="photoname"
								data-valmsg-replace="true"></span></li>
						...
						<ul class="DialabelWidth">
							<li class="editor-label"><label for="photoType">系统级别</label>
							</li>
							<c:if test="${empty photoSample.id}">
								<li class="editor-field"><select id="photoType"
									disabled="disabled" name="photoType">
										<option
											<c:if test="${photoSample.photoType == 1}">selected=selected</c:if>
											value="1">系统级别</option>
										<option
											<c:if test="${photoSample.photoType == 2}">selected=selected</c:if>
											value="2">部门级别</option>
										<option
											<c:if test="${photoSample.photoType == 3}">selected=selected</c:if>
											value="3">窗口级别</option>
								</select></li>
							</c:if>
							<c:if test="${not empty photoSample.id}">
								<li class="editor-field"><c:if
										test="${photoSample.photoType == 1}">
										<input value="系统级别" readonly="readonly">
									</c:if> <c:if test="${photoSample.photoType == 2}">
										<input value="部门级别" readonly="readonly">
									</c:if> <c:if test="${photoSample.photoType == 3}">
										<input value="窗口级别" readonly="readonly">
									</c:if></li>
							</c:if>
							<li class="editor-validation"><span
								class="field-validation-valid" data-valmsg-for="photoType"
								data-valmsg-replace="true"></span></li>
						</ul>
						<div class="clear">
							<ul class="DialabelWidth" style="width:auto;margin-left: -18px;">
								...
								<li class="editor-field"><textarea cols="20"
										data-val="true" data-val-length="样图描述不能超过255字符"
			...
			<div class="fPptions" style="text-align: center;margin-left: -79%;">
				<input type="button" id="submitBtn" value="提交"
					style="height: 30px;width: 70px;line-height: 25px;"
					class="btnLightBlue btn" /> <a href="javascript:void(0);"
					title="返回" class="btnLightBlue btn back"
					style="width: 50px; height: 30px;" id="reback"><i class="i"></i>返回</a>
			</div>
		</form>
	</div>