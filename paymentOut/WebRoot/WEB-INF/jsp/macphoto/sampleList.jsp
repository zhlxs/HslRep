...
<script type="text/javascript">
	rwpTemp.deptbaritems = {
		items : [ {
			...
			min : 0,
			max : 0.5,
			url : '${ctx}/macPhotoController/addSample',
			link : true
		}, {
			text : '修改',
			click : rwpListLayout.gridToolButtonClick.itemclick,
			...
			max : 1,
			link : true,
			url : '${ctx}/macPhotoController/editSample'
		}/* , {
			text : '删除',
			click : rwpListLayout.gridToolButtonClick.itemclick,
			icon : 'delete',
			...
			confirm : true,
			postajax : true,
			url : '${ctx}/macPhotoController/deleteSample'
		}  */]
	};
				...
				.rwpUIDatagrid(
						{
							columns : [
									...
									},
									{
										...
										render : function(rowdata) {
											...
											if (rowdata.id > 0) {
												if (rowdata.isvalid)
													html += '<a href="${ctx}/macPhotoController/showSample?isvalid=0&id='
															+ rowdata.id
															+ '" rwpOpts="{ isajax: true }" >失效</a>';
												else
													html += '<a href="${ctx}/macPhotoController/showSample?isvalid=1&id='
															+ rowdata.id
							...
							url : '${ctx}/macPhotoController/sampleJson',
							height : '99%',
	...
	$(function() {
		$(".pageHeader").html("<i class='i'></i>设备-样图管理");
		$("#deptform").rwpUIForm({
			inputWidth : 150,
			isSearch : true
		});
