...
; (function ($, window, document, undefined) {   
    ...
    var pluginName = 'rwpUIDateinputTWO';   //控件名称

    var rwpUIDateinputTWO = function (element, options) {
        this.element = element;
        this.$element = $(element);

        var opt = $.extend({}, $.rwpUI.Dateinput.defaults, options);  //合并已赋值参数
        opt.id = opt.id || $.rwpUI.guid('rwpDateinputTWO_');
        this.options = opt;
    ...
    rwpDateinputTWOTWO.prototype = {
        ...
        _setValue: function (date) {  //赋值
            ...
            if (date) {
                if (opt.time)
                    dateStr = rwpDateTime.format(date, 'yyyy-MM-dd 23:59');
                else
                    dateStr = rwpDateTime.format(date, 'yyyy-MM-dd 23:59');
            }
...
//        },
    ...
    $.rwpUI.Dateinput = function (element, options) {
        (new rwpDateinputTWOTWO(element, options));
    };
    ...
    $.rwpUI.Dateinput.fn = rwpDateinputTWO;  //指向rwpDateinputTWO
