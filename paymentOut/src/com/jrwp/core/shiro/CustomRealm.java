...
public class CustomRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		System.out.println("为当前登录的Subject授予角色和权限");
		...
		// 从数据库中获取当前登录用户的详细信息
		Core_User user = userService.getByUserName(currentUsername);
		if (null != user) {
			// 实体类User中包含有用户角色的实体类信息
			if (null != user.getRoles() && user.getRoles().size() > 0) {
				// 获取当前登录用户的角色
				for (Core_Role role : user.getRoles()) {
					roleList.add(role.getRoleName());
					// 实体类Role中包含有角色权限的实体类信息
					if (null != role.getAuths() && role.getAuths().size() > 0) {
						// 获取权限
						for (Core_Auth auth : role.getAuths()) {

							if (null != auth.getActions()
									&& auth.getActions().size() > 0) {
								List<Core_ActionForAuth> actions = auth
										.getActions();
								for (Core_ActionForAuth action : actions) {
									if (!StringUtils.isEmpty(action
											.getControlName())
											&& !StringUtils.isEmpty(action
													.getActionName())) {
										permissionList.add(action
												.getControlName()
												+ ":"
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		System.out.println("验证当前登录的Subject");
		UsernamePasswordToken token = (UsernamePasswordToken) authcToken;
		Core_User user = userService.getByUserName(token.getUsername());
		if (user != null) {
	...
	public void clearAllCachedAuthorizationInfo() {
		System.out.println("清除所有用户授权信息缓存.");
